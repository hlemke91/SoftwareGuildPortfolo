...
<div class="container">
    <div class ="col-centered">
        ...
        <form action="updateOrg" method="put">
            Name:<input type="text" class="form-control" value="${org.name}" name="name"><br>
            Description:<input type="text" class="form-control" value="${org.description}" name="description"><br>
            Headquarters: Previously was ${org.hq.name}<select  class="form-control"name="hq">
                <c:forEach var="loc" items="${locs}">
                    <option value="${loc.ID}">${loc.name}</option>
                </c:forEach>
            </select><br>
            Supers: Previously was<c:forEach var="superguy" items="${org.roster}"> ${superguy.name} </c:forEach>(hold control to select multiple)<select class="form-control" multiple="multiple" name="superID">
